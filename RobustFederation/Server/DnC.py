...
class DnC(ServerMethod):
    ...
    def server_update(self, **kwargs):
        ...
        bad_client_num = int(self.cfg.attack.bad_client_rate * len(online_clients_list)) # Require the number of bad clients
        benign_ids = []
        ...
        self.agg_parts(online_clients_list=benign_ids, nets_list=nets_list,
                       global_net=global_net, freq=[1/len(benign_ids) for i in range(len(benign_ids))], except_part=[], global_only=False)
