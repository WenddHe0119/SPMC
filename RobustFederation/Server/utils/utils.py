...
def trimmed_mean_resnet(users_grads, users_count, corrupted_count):
    num_clients, num_params = users_grads.shape
    number_to_consider = num_clients - corrupted_count - 1

    # Step 1: 计算每一列的中位数，shape = (num_params,)
    med = np.median(users_grads, axis=0)

    # Step 2: 广播中位数，计算差值矩阵，shape = (num_clients, num_params)
    diff = users_grads - med

    # Step 3: 计算每个维度上差值的绝对值并排序
    # argsort 第二次排序保证稳定性，返回前 number_to_consider 行（最小差值的客户端）
    sorted_indices = np.argsort(np.abs(diff), axis=0)[:number_to_consider]

    # Step 4: 对每个参数维度，选出前 number_to_consider 个客户端的值进行平均
    # 利用高级索引提取这些值
    col_indices = np.arange(num_params)
    trimmed_values = diff[sorted_indices, col_indices[None, :]]
    trimmed_mean = np.mean(trimmed_values, axis=0)

    # Step 5: 加回中位数
    result = trimmed_mean + med

    return result


def bulyan(users_grads, users_count, corrupted_count, resnet_flag = False):
    ...
    if resnet_flag:
        return trimmed_mean_resnet(np.array(selection_set), len(selection_set), 2 * corrupted_count)
    else:
...
def fools_gold_weight(this_delta, summed_deltas, sig_features_idx, model, topk_prop=0, importance=False, importanceHard=False, clip=0):
    # Take all the features of sig_features_idx for each clients
    sd = summed_deltas.copy()
    sig_filtered_deltas = np.take(sd, sig_features_idx, axis=1)
    epsilon = 1e-5
