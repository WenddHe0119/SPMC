...
class ServerMethod(nn.Module):
    ...
    def agg_layers_parts(self, **kwargs):
        freq = kwargs['freq']
        online_clients_list = kwargs['online_clients_list']
        nets_list = kwargs['nets_list']
        global_net = kwargs['global_net']
        ...
        if 'use_additional_net' in kwargs:
            ...
            additional_net_list = kwargs['additional_net_list']
            additional_freq = kwargs['additional_freq']
        for layer_name, layer_param in global_net.state_dict().items():
            # used_net_para = {}
            first = True
            for index, net_id in enumerate(online_clients_list):
                w = layer_weight[layer_name][net_id]
                net = nets_list[net_id]
                net_para = net.state_dict()
                ...
                is_in = False
                for part_str_index in range(len(except_part)):
                    if except_part[part_str_index] == layer_name:
                        is_in = True
                        break

                if not is_in:
                    used_net_para[layer_name] = net_para[layer_name]

                if first:
                    first = False
                    # for key in used_net_para:
                    global_w[layer_name] = used_net_para[layer_name] * w
                else:
                    # for key in used_net_para:
                    global_w[layer_name] += used_net_para[layer_name] * w
    ...
    def agg_param_parts(self, **kwargs):
        # freq = kwargs['freq']
        online_clients_list = kwargs['online_clients_list']
        nets_list = kwargs['nets_list']
        global_net = kwargs['global_net']
        ...
        for index, net_id in enumerate(online_clients_list):
            net = nets_list[net_id]
            net_para = net.state_dict()
            hessian_freq = hessian_ma[net_id]
            ...
            if first:
                first = False
                for key in used_net_para:
                    global_w[key] = used_net_para[key] * hessian_freq[key]
            else:
                for key in used_net_para:
                    global_w[key] += used_net_para[key] * hessian_freq[key]
