...
class FedLeaCIFAR10(SingleDomainDataset):
    ...
    def get_data_loaders(self):
        ...
        #note download=false
        if self.cfg.DATASET.val_aug == 'weak':
            vali_dataset = MyCIFAR10(root=single_domain_data_path(), train=True,
                                      download=True, transform=self.weak_transform)
        elif self.cfg.DATASET.val_aug == 'strong':
            vali_dataset = MyCIFAR10(root=single_domain_data_path(), train=True,
                                      download=False, transform=self.strong_transform)
        test_transform = transforms.Compose(
            [transforms.ToTensor(), self.get_normalization_transform()])
        test_dataset = CIFAR10(single_domain_data_path(), train=False,
                               download=False, transform=test_transform)
        self.partition_label_skew_loaders(train_dataset, test_dataset,vali_dataset)
