...
def row_into_parameters(row, parameters):
    ...
    for param in parameters:
        ...
        param.data[:] = torch.from_numpy(current_data.reshape(param.shape)).to(param.data.dtype)
        offset += new_size

def row_into_state_dict(row: np.ndarray, state_dict: dict) -> dict:
    """
    将一个扁平化 numpy 向量 row 依次写入 state_dict 中每个 tensor 的形状，
    返回一个新的 dict，可用于 load_state_dict。

    Args:
        row (np.ndarray): 一维参数向量
        state_dict (dict): PyTorch 模型的 state_dict()

    Returns:
        new_state_dict (dict): 新的参数 dict（结构与原始 state_dict 一致）
    """
    new_state_dict = {}
    offset = 0
    ...
    for name, param in state_dict.items():
        if not isinstance(param, torch.Tensor):
            raise TypeError(f"Expected Tensor in state_dict, but got {type(param)} for key '{name}'")

        numel = param.numel()
        new_data = row[offset:offset + numel].reshape(param.shape)
        new_tensor = torch.from_numpy(new_data).to(param.dtype)
        new_state_dict[name] = new_tensor
        offset += numel

    if offset != len(row):
        raise ValueError(f"row length mismatch: used {offset} but got {len(row)}")
